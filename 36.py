...
def print_operation_table(operation, num_rows=6, num_columns=6):
    for i in range(1, num_rows + 1):
        for j in range(1, num_columns + 1):
            result = operation(i, j)  # вычисляем значение по номеру строки и столбца
            print(result, end="\t")  # печатаем значение в строку с разделителем табуляции
        print()  # переходим на новую строку
# пример
def multiply(x, y):
    return x * y

print_operation_table(multiply, 5, 7)